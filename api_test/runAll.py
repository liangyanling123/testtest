...
summary_path = pro_dir + '/result/' + summary_name
...
def generate_report(path, title, description):
    ...
    try:
        if report_name in os.listdir(pro_dir + '/result'):
            ...
            if common.find_word(path, title) and common.find_word(path, description):
...
if __name__ == '__main__':
    ...
    if generate_report(report_path, report_title, report_description) is True:  # 执行测试完成，报告生成
        if common.upload_file(report_name) is False:  # 文件上传到服务器失败，将文件放在邮件附件中发送出去
            summary_html.write(content)  # 详细测试报告未能上传到服务器，简要报告不包含访问地址
            summary_html.close()
            send_email.send_email(False, None, report_path, summary_path, True)
        else:
            summary_html.write(content+content_report_path)  # 详细测试报告上传到服务器，简要报告包含访问地址
            summary_html.close()
            send_email.send_email(True, remote_path, report_path, summary_path, False)  # 邮件不发送附件
